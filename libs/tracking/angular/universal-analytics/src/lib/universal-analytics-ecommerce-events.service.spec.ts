...
import { GtmEvent, UniversalAnalyticsEcommerceProductImpressionsEvent } from '@multi-step-funnels/tracking/tracking-models';
...
describe('UniversalAnalyticsEcommerceEventsService', () => {
  ...
  beforeEach(() => {
    ...
    trackingGoogleTagManagerService = TestBed.inject(TrackingGoogleTagManagerService);
  ...
  describe('#triggerProductImpressionsEvent', () => {
    let spyOnTriggerProductImpressionsEvent: jest.SpyInstance<void, [productImpressionsEvent: UniversalAnalyticsEcommerceProductImpressionsEvent]>;
    let spyOnTransformProductImpressionEventToGtmEvent: jest.SpyInstance<any, unknown[]>;
    let spyOnPushToDataLayer: jest.SpyInstance<void, [obj: GtmEvent]>;

    beforeEach(() => {
      spyOnTriggerProductImpressionsEvent = jest.spyOn(service, 'triggerProductImpressionsEvent');
      spyOnTransformProductImpressionEventToGtmEvent = jest.spyOn(service as any, 'transformProductImpressionEventToGtmEvent');
      spyOnPushToDataLayer = jest.spyOn(trackingGoogleTagManagerService, 'pushToDataLayer');
    ...
    it('should be called with an argument of type UniversalAnalyticsEcommerceProductImpressionsEvent', ()=> {
      expect(spyOnTriggerProductImpressionsEvent).toBeCalledWith(productImpressionEvent);
    });
    ...
    it('should push an "angularProductImpressions" event to the dataLayer', () => {
      expect(dataLayer[dataLayer.length -1].event).toBe('angularProductImpressions');
    });
  });
